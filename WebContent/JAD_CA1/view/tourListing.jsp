...
<body>
  ...
  <section class="destinations">
    <h3 class="title">Tour Listing:</h3>
    <p>Check out the attractive tour listings!</p>
    <% 
		if (request.getParameter("errCode") != null) {
		        out.println("Tour does not exist!<br>");
		}
	%>
...
//Create arrayList for tourID
ArrayList<Integer> tourID = new ArrayList<Integer>();
		
	if(session.getAttribute("sessTourID") != null){
		tourID = (ArrayList<Integer>) session.getAttribute("sessTourID");
	}
  ...
  <%
	...
	<%
	//Display tours based on categoryID if tourID does not exist (For additional search feature)
	if(session.getAttribute("sessTourID") == null && request.getParameter("catID") != null){
		int catSelection = Integer.parseInt(request.getParameter("catID"));
		
		for(Tours t : tours){
			if(t.getCategoryID() == catSelection){	
			%>
			<a href="../view/tourDetails.jsp?tourID=<%= t.getId() %>">
			        <div class="row">
				    <div class="col-2">
				      <img src=<%= t.getImgLocation() %> alt="" />
				      <h4 align="center"><%= t.getTitle() %></h4>
				      <div class="rating">
	...
	}else if(session.getAttribute("sessTourID") != null){ //If tourID exists... (When search input is not empty)
		
		for(Tours t : tours){
		
		for(int i = 0; i<tourID.size(); i++){
			
			if(t.getId() == tourID.get(i)){
			%>
			<a href="../view/tourDetails.jsp?tourID=<%= t.getId() %>">
			        <div class="row">
				    <div class="col-2">
	...
	//invalidate session
		session.invalidate();
	}else{ //Tour searched does not exist
		out.print("");
	}%>
